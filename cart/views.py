        ...
        if quantity.strip() is not None:
            try:
                if cart.objects.filter(product_id=product_id).exists():
                    ...
                    if add.book_quantity > product_id.quantity or add.book_quantity > 10 or add.book_quantity < 1:
                        print('2',product_id)
                    ...
                    if Product_Offer.objects.filter(produc_id=product_id,is_active = True).exists():
                        offer =True
                        product = Product_Offer.objects.get(produc_id=product_id,is_active = True)
                    ...
                    return redirect("single_prodect",id)
                if product_id.quantity < int(quantity) or int(quantity) > 10 or  int(quantity) < 1:
                    print('1',product_id)
                    messages.success(request, "We did't have that much quantity")
                    return redirect("single_prodect",id)
                    ...
                    cart_obj =cart(user_id=user_id,product_id=product_id,book_quantity=int(quantity),cart_total=int(quantity)*amount,current_price = amount)
                    cart_obj.save()
        ...
        if cart.objects.filter(product_id=product).exists():
            ...
            if offer:
                ...
                amount -=minus
                    ...
                    # offeer end
            cart_obj =cart(user_id=user_id,product_id=product,book_quantity=1,cart_total=1*amount,current_price = amount)
            print('add to cart first one')
            cart_obj.save()
        print('1.1',product_id)
        # messages.success(request, 'added to cart')
