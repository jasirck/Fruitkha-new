    ...
    try:
        if request.method == "POST":
            ...
            if Coupon.objects.filter(code = code,active = True).exists():
                ...
                return render(request,'add_coupon.html',{'users':count,'pro':count_pro,'ord':order_count})
            Coupon.objects.create(coupon_name = Coupon_name,code = code,discount = discount,minimum_amount = minimum_amount,start_time = start_time,expiration_time = expiration_time,msg = msg)
...
def check_coupon_validity(request):
    if request.method == 'POST' and request.is_ajax():
        ...
        try:
            ...
            amount = cart.objects.filter(user_id=user_id.id).aggregate(subtotal=Sum('cart_total'))
            now = timezone.now()
            start_time = coupon.start_time
            expiration_time = coupon.expiration_time
            if coupon.active and coupon.is_expired and start_time < now < expiration_time:
                if order.objects.filter(user=user_id, coupon_id=coupon.id).exists():
                    ...
                    return JsonResponse({'valid': False, 'message': f'Minimum amount is {coupon.minimum_amount}.'})
                else:
                    request.session['coupon_code'] = coupon_code
                    return JsonResponse({'valid': True, 'message': 'Coupon is valid.'})
            else:
                return JsonResponse({'valid': False, 'message': 'Coupon is either inactive or expired.'})
