    ...
    try:
        ...
        if request.method == "POST":
            ...
            if Product_Offer.objects.filter(produc_id = product).exists():
                ...
                return render(request,'add_product_offer.html',{'product':product_all,'users':count,'pro':count_pro,'ord':order_count})#
            product_id = myprodect.objects.get(id = product)
            Product_Offer.objects.create(produc_id = product_id,percentage = discount,start_date = start_time,end_date = expiration_time)
...
def product_offer(request):
    ...
    return render(request,'product_offer.html',{'offer':offer,'users':count,'pro':count_pro,'ord':order_count})
...
def add_category_offer(request):
    ...
    try:
        if request.method == "POST":
            ...
            start_time = request.POST.get('staring')
            expiration_time = request.POST.get('ending')
            if Category_Offer.objects.filter(produc_id = category).exists():
                messages.info(request, 'Offer Exists')
                return render(request,'add_product_offer.html',{'category':category_all,'users':count,'pro':count_pro,'ord':order_count})#
            category_id = AdminCategory.objects.get(id = category)
            Category_Offer.objects.create(category_id = category_id,percentage = discount,start_date = start_time,end_date = expiration_time)
            messages.info(request, 'Offer Created')
            return render(request,'add_Category_offer.html',{'category':category_all,'users':count,'pro':count_pro,'ord':order_count})
    except Exception as e:
            messages.info(request, 'somthig error!')
            print(e)
            return render(request,'add_category_offer.html',{'category':category_all,'users':count,'pro':count_pro,'ord':order_count})
...
def category_offer(request):
    ...
    return render(request,'category_offer.html',{'offer':offer,'users':count,'pro':count_pro,'ord':order_count})
