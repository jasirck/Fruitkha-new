...
class Product_Offer(models.Model):
    ...
    produc_id =models.ForeignKey(myprodect,  on_delete=models.CASCADE,null=True, blank=True)
...
class Category_Offer(models.Model):
    id = models.BigAutoField(primary_key=True)
    category_id = models.ForeignKey(AdminCategory, on_delete=models.CASCADE,null=True, blank=True)
    percentage = models.FloatField(null=True, blank=True, default = 0)
    start_date = models.DateTimeField(null=True, blank=True,  auto_now=False, auto_now_add=False)
    end_date = models.DateTimeField(null=True, blank=True,  auto_now=False, auto_now_add=False)
    is_active = models.BooleanField(default = True)
